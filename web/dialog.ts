...
class Dialog {
	...
	public showConfirmation(message: string, actionName: string, actioned: () => void, target: DialogTarget | null) {
		this.show(DialogType.Form, message, actionName, t('取消'), () => {
			this.close();
			actioned();
		}, null, target);
	}
	 ...
	 */
	public showForm(message: string, inputs: ReadonlyArray<DialogInput>, actionName: string, actioned: (values: DialogInputValue[]) => void, target: DialogTarget | null, secondaryActionName: string = t('取消'), secondaryActioned: ((values: DialogInputValue[]) => void) | null = null, includeLineBreak: boolean = true) {
		...
		if (textRefInput > -1) {
			...
			dialogInput.addEventListener('keyup', () => {
				...
				if (alterClass(this.elem, CLASS_DIALOG_INPUT_INVALID, !noInput && invalidInput)) {
					dialogAction.title = invalidInput ? (UI_LANG === 'en' ? ('Unable to ' + actionName + ', one or more invalid characters entered.') : ('无法' + actionName + '，输入包含一个或多个非法字符。')) : '';
				}
			});
		}
	...
	public showMessage(html: string) {
		this.show(DialogType.Message, html, null, t('关闭'), null, null, null);
	}
	...
	public showError(message: string, reason: GG.ErrorInfo, actionName: string | null, actioned: (() => void) | null) {
		this.show(DialogType.Message, '<span class="dialogAlert">' + SVG_ICONS.alert + t('错误：') + message + '</span>' + (reason !== null ? '<br><span class="messageContent errorContent">' + escapeHtml(reason).split('\n').join('<br>') + '</span>' : ''), actionName, t('关闭'), () => {
			this.close();
	...
	public showActionRunning(action: string) {
		this.show(DialogType.ActionRunning, '<span class="actionRunning">' + SVG_ICONS.loading + action + ' ...</span>', null, t('关闭'), null, null, null);
	}
