...
class Graph {
	...
	private showTooltip(id: number, vertexScreenY: number) {
		...
		let html = '<div class="graphTooltipTitle">提交 ' + abbrevCommit(this.commits[id].hash) + '</div>';
		if (this.commitHead !== null && typeof this.commitLookup[this.commitHead] === 'number') {
			html += '<div class="graphTooltipSection">该提交' + (childrenIncludesHead ? '' : '<b><i>未</i></b>') + '包含于 <span class="graphTooltipRef">HEAD</span></div>';
		}
		if (heads.length > 0 || remotes.length > 0) {
			...
			html += '<div class="graphTooltipSection">分支：' + getLimitedRefs(htmlRefs) + '</div>';
		}
		if (tags.length > 0) {
			let htmlRefs = tags.map((tag) => '<span class="graphTooltipRef">' + escapeHtml(tag) + '</span>');
			html += '<div class="graphTooltipSection">标签：' + getLimitedRefs(htmlRefs) + '</div>';
		}
		if (stashes.length > 0) {
			let htmlRefs = stashes.map((stash) => '<span class="graphTooltipRef">' + escapeHtml(stash) + '</span>');
			html += '<div class="graphTooltipSection">贮藏：' + getLimitedRefs(htmlRefs) + '</div>';
		}
